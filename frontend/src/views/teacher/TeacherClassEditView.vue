...
const fetchDetail = async () => {
  if (!Number.isFinite(classId) || classId <= 0) {
    await ui.alert('班级ID无效', { tone: 'error' })
  ...
  try {
    ...
    await ui.alert('加载班级信息失败', { tone: 'error' })
...
const handleUpdate = async () => {
  ...
  try {
    ...
    await ui.alert('更新班级失败', { tone: 'error' })
  } finally {
